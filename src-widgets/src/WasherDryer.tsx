...
export default class WasherDryer extends Generic<WasherDryerRxData, WasherDryerState> {
    ...
    async propertiesUpdate(): Promise<void> {
        ...
        // read object itself
        const fetchedObject = await this.props.context.socket.getObject(this.statusOID);
        const object: ioBroker.StateObject = {
            common: (fetchedObject?.common as ioBroker.StateCommon) || ({} as ioBroker.StateCommon),
            _id: fetchedObject?._id || this.statusOID,
            native: {},
            type: 'state',
        };
