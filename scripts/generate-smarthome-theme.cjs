...
const presetNames = ['ocean', 'daytime', 'midnight', 'plum', 'happymode', 'graphite'];
const banner = '/* Generated from themes/*.json by npm run theme:generate. Do not edit. */\n';
let tokenCss = `${banner}/* Namespaced tokens inherit into widgets and project layouts. */\n`;
let projectCss = `${banner}/* Explicit project-wide compatibility opt-in; includes the editor's MUI CSS. */\n@import url('./smarthome.css');\n`;

for (const name of presetNames) {
    const preset = JSON.parse(fs.readFileSync(path.join(publicDir, `themes/${name}.json`), 'utf8'));
    const theme = createTheme({ ...preset, cssVariables: true });
    const p = theme.palette;
    const s = theme.smartHome;
    const tokens = {
        'color-scheme': preset.defaultColorScheme,
        bg: p.background.default,
        surface: p.background.paper,
        'surface-2': s.surfaceRaised,
        floor: p.background.default,
        wall: s.wall,
        'floorplan-fill-opacity': s.floorplan.fillOpacity,
        'floorplan-inner-opacity': s.floorplan.innerWallOpacity,
        'floorplan-outer-width': s.floorplan.outerWallWidth,
        'floorplan-inner-width': s.floorplan.innerWallWidth,
        'floorplan-shadow': s.floorplan.shadow,
        ...
        'blind-scene-background': s.blindScene.background,
        'blind-scene-frame': s.blindScene.frame,
        'blind-scene-frame-inner': s.blindScene.frameInner,
        'blind-scene-glass-top': s.blindScene.glassTop,
        'blind-scene-glass-bottom': s.blindScene.glassBottom,
        'blind-scene-hill': s.blindScene.hill,
        'blind-scene-hill-mid': s.blindScene.hillMid,
        'blind-scene-hill-deep': s.blindScene.hillDeep,
        'blind-scene-person': s.blindScene.person,
        'blind-scene-sun': s.blindScene.sun,
        'blind-scene-slat': s.blindScene.slat,
        'blind-scene-slat-line': s.blindScene.slatLine,
        focus: p.mode === 'light' ? p.secondary.main : p.secondary.light,
    ...
    const declarations = Object.entries(tokens)
        ...
        .join('\n');
    const selector = name === 'ocean' ? ':where(:root)' : `:root[data-sh-theme="${name}"]`;
    tokenCss += `${selector} {\n${declarations}\n}\n`;
    ...
    const aliasCss = Object.entries(aliases)
        .map(([key, value]) => `    ${key}: ${value};`)
        .join('\n');
    const projectSelector = name === 'ocean' ? ':root' : `:root[data-sh-theme="${name}"]`;
    projectCss += `\n${projectSelector} {\n${paletteCss}\n${aliasCss}\n}\n`;
}
...
console.log(
    process.argv.includes('--check') ? 'Theme CSS matches all presets.' : 'Generated theme CSS for all presets.',
);
